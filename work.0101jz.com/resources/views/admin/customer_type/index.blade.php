...
@push('footlast')
	<script type="text/javascript">
        ...
        const SHOW_URL = "";// {{url('customer_type/info/')}}/";//显示页面地址前缀 + id
        const EDIT_URL = "{{url('admin/customer_type/add/')}}/";//修改页面地址前缀 + id
        const DEL_URL = "{{ url('api/admin/customer_type/ajax_del') }}";//删除页面地址
        const BATCH_DEL_URL = "";// {{ url('api/manage/customer_type/ajax_del') }}";//批量删除页面地址
        const EXPORT_EXCEL_URL = ""; // {{ url('manage/customer_type/add/0') }}"; //"{{ url('api/manage/customer_type/export') }}";//导出EXCEL地址
        const IMPORT_EXCEL_URL =""; // "{{ url('manage/customer_type/add/0') }}"; //"{{ url('api/manage/customer_type/import') }}";//导入EXCEL地址
