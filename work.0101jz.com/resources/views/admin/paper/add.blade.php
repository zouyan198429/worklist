...
@push('footlast')
	<script type="text/javascript">
        var SAVE_URL = "{{ url('api/admin/paper/ajax_save') }}";// ajax保存记录地址
        var LIST_URL = "{{url('admin/paper')}}";//保存成功后跳转到的地址

        var ID_VAL = "{{ $id or 0 }}";// 当前id值
        var AJAX_SUBJECT_URL = "{{ url('api/admin/paper/ajax_get_subject') }}";// ajax初始化试题地址
        var AJAX_UPDATE_SUBJECT_URL = "{{ url('api/admin/paper/ajax_update_subject') }}";// ajax更新试题地址
        var AJAX_SUBJECT_ADD_URL = "{{ url('api/admin/paper/ajax_add_subject') }}";// ajax添加试题地址
        var SELECT_SUBJECT_URL = "{{ url('admin/subject/select') }}";// 选择试题地址
        var DYNAMIC_BAIDU_TEMPLATE = "baidu_template_data_list";//百度模板id
        var DYNAMIC_TABLE_BODY = "data_list";//数据列表class

	</script>
	<script src="{{ asset('/js/admin/lanmu/paper_edit.js') }}"  type="text/javascript"></script>
@endpush