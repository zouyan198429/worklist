...
<script type="text/javascript">
	...
	var AJAX_URL = "{{ url('api/weixiu/customer/ajax_alist') }}";//ajax请求的url
	var ADD_URL = "{{ url('weixiu/customer/add/0') }}"; //添加url
	const SHOW_URL = "{{url('weixiu/customer/info/')}}/"; //显示页面地址前缀 + id
    const SHOW_URL_TITLE = "" ;// 详情弹窗显示提示
    var SHOW_CLOSE_OPERATE = 0 ;// 详情弹窗operate_num关闭时的操作0不做任何操作1刷新当前页面2刷新当前列表页面
	var EDIT_URL = "{{url('weixiu/customer/add/')}}/"; //修改页面地址前缀 + id
	var DEL_URL = "{{ url('api/weixiu/customer/ajax_del') }}";  //删除页面地址
	var BATCH_DEL_URL = "{{ url('api/weixiu/customer/ajax_del') }}"; //批量删除页面地址
...
<script src="{{ asset('js/weixiu/lanmu/customer.js') }}"  type="text/javascript"></script>
@endpush
