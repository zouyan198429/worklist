...
@push('footlast')
	<script type="text/javascript">
        var INIT_EXAM_URL = "{{ url('api/weixiu/exam/ajax_init_exam') }}";// ajax在线考试初始化地址
        var SAVE_URL = "{{ url('api/weixiu/exam/doing_ajax_save') }}";// ajax保存记录地址
        var EXAM_STAFF_ID = '{{ $id or 0 }}';// 考试员工表id
        var WIN_URL = "{{ url('weixiu/exam/win') }}/" + EXAM_STAFF_ID;// 交卷后跳转地址

        var DYNAMIC_BAIDU_TEMPLATE = "baidu_template_data_list";//百度模板id
        var DYNAMIC_TABLE_BODY = "data_list";//数据列表class
        var EXAM_END_TIME = "{{ $exam_end_time or '' }}";// 结束时间

        var SUBMIT_FORM = true;//防止多次点击提交
	</script>
	<script src="{{ asset('/js/weixiu/lanmu/exam_doing.js') }}"  type="text/javascript"></script>
@endpush