...
function initList(){
    // 获得选中的试题id 数组
    var SELECTED_IDS = parent.getSelectedSubjectIds(SELECTED_SUBJECT_TYPE);
    ...
    $('#data_list').find('tr').each(function () {
        ...
        var subject_id = checkedObj.val();
        console.log('subject_id', subject_id);
        if(SELECTED_IDS.indexOf(subject_id) !== -1){// 已选
            trObj.find('.add').hide();
            trObj.find('.del').show();
...
var otheraction = {
    addBatch : function(obj){// 批量增加试题
        ...
        var index_query = layer.confirm('确定增加选中记录？', {
            ...
            parent.addSubject(SELECTED_SUBJECT_TYPE, ids);
            initList();
    ...
    add : function(id){// 增加单个试题
        var index_query = layer.confirm('确定增加当前记录？', {
            ...
            parent.addSubject(SELECTED_SUBJECT_TYPE, id);
            initList();
            layer.close(index_query);
        }, function(){
        });
    ...
    del : function(id){// 取消
        var index_query = layer.confirm('确定取消当前记录？', {
            ...
            parent.removeSubject(SELECTED_SUBJECT_TYPE, id);
            initList();
            layer.close(index_query);
        }, function(){
        });
